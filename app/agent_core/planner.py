...
_TRIVIAL_RE = re.compile(
    r"^(oi|ol[áa]|e a[ií]|eae|hey|hello|hi)\b|"
    r"\b(o que (é|significa|faz)|qual (é|a fun[çc][aã]o)|como funciona|defina)\b",
    re.IGNORECASE,
)
...
class Planner:
    ...
    def plan(self, user_input: str, tool_names: list[str], session_id: str = "global") -> dict:
        ...
        # Fast-path: trivial inputs don't need an LLM call
        if _TRIVIAL_RE.search(user_input):
            return {"plan": "DIRETO: pergunta trivial detectada por padrão.", "direct": True, "raw": {}}
