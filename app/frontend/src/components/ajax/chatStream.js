...
export async function streamChat({ API, session_id, message, provider, model, onEvent, onDone, onError }) {
  ...
  // Timeout de 5 minutos para Ollama local em maquinas com pouca RAM
  ...
  try {
    ...
    if (err.name === "AbortError") {
      onError(new Error("Tempo esgotado — o modelo demorou mais de 5 minutos. Tente OpenRouter (mais rapido) ou feche outros programas."));
    } else {
      onError(err);
    }
  }
