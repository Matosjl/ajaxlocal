...
def make_router(provider: str = "openrouter", model: str | None = None) -> LLMRouter:
    if provider == "ollama":
        return LLMRouter(provider="ollama", model=model or os.environ.get("OLLAMA_MODEL", "qwen2.5-coder:1.5b"),
...
async def chat_stream(body: ChatRequest):
    ...
    async def event_stream():
        ...
        while True:
            try:
                ev = await asyncio.wait_for(queue.get(), timeout=300.0)
            except asyncio.TimeoutError:
                yield f"data: {json.dumps({'type': 'error', 'message': 'Tempo esgotado no servidor — o modelo demorou muito. Use OpenRouter ou feche outros programas.'}, ensure_ascii=False)}\n\n"
                break
            if ev is None:
                break
